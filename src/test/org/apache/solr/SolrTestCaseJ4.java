...
/**
 * A junit4 Solr test harness that extends LuceneTestCaseJ4. Unlike AbstractSolrTestCase, a new core is not created for
 * each test method.
 * 
 */
public abstract class SolrTestCaseJ4 {
  ...
  /**
   * Call initCore in @BeforeClass to instantiate a solr core in your test class. deleteCore will be called for you via
   * SolrTestCaseJ4 @AfterClass
   */
  ...
  static {
    ...
    if (s == null) throw new RuntimeException("To run tests, you need to define system property 'tempDir' or 'java.io.tmpdir'.");
    TEMP_DIR = new File(s);
    TEMP_DIR.mkdirs();
  }
  ...
  /**
   * Subclasses that override tearDown can optionally call this method to log the fact that the tearDown process has
   * started. This is necessary since subclasses will want to call super.tearDown() at the *end* of their tearDown
   * method.
   */
  ...
  /**
   * Shuts down the test harness, and makes the best attempt possible to delete dataDir, unless the system property
   * "solr.test.leavedatadir" is set.
   */
  ...
  /**
   * Validates a query matches some JSON test expressions and closes the query. The text expression is of the form
   * path:JSON. To facilitate easy embedding in Java strings, the JSON can have double quotes replaced with single
   * quotes.
   * 
   * Please use this with care: this makes it easy to match complete structures, but doing so can result in fragile
   * tests if you are matching more than what you want to test.
   * 
   **/
   ...
   *          0th and Even numbered args are fields names odds are field values.
   * @see #add
   * @see #doc
   */
   ...
   *          the Document to add
   * @param args
   *          0th and Even numbered args are param names, Odds are param values.
   * @see #add
   * @see #doc
   */
